...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	...
	/** 
	 * Index of column productId on view
	 */
	protected static final int COLUMN_VIEW_PRODUCT_ID = 1;

	/** 
	 * Index of column productName on view
	 */
	...
	/** 
	 * Index of column storeId on view
	 */
	protected static final int COLUMN_VIEW_STORE_ID = 4;

	/** 
	 * Index of column storeName on view
	 */
	...
	/** 
	 * Index of column unityValue on view
	 */
	 ...
	 */
	public boolean update(KardexPk pk, KardexDto dto) throws KardexDaoException
	{
		long t1 = System.currentTimeMillis();
		// declare variables
		final boolean isConnSupplied = (userConn != null);
	 ...
	 * @param userConn
	 */
	public KardexDao(final java.sql.Connection userConn)
	{
	 ...
	 * Fetches multiple rows from the result set
	 */
	protected KardexDto[] fetchKardexDto(ResultSet rs) throws SQLException
	{
	 ...
	 * Populates a DTO with data from a ResultSet
	 */
	protected void populateKardexView(KardexView kardexView, ResultSet rs) throws SQLException
	{
            kardexView.setProductId( rs.getInt( COLUMN_VIEW_PRODUCT_ID ) );
            kardexView.setProductName(rs.getString( COLUMN_VIEW_PRODUCT_NAME ) );
            kardexView.setStoreId( rs.getInt( COLUMN_VIEW_STORE_ID ) );
            kardexView.setStoreName(rs.getString( COLUMN_VIEW_STORE_NAME ) );
            kardexView.setQuantity( rs.getDouble( COLUMN_VIEW_QUANTITY ) );
            kardexView.setUnityDescription(rs.getString( COLUMN_VIEW_UNITY_DESCRIPTION ) );
            kardexView.setUnityValue( rs.getDouble( COLUMN_VIEW_UNITY_VALUE ) );	
	}
    ...
    public KardexView[] getKardexView(int productId, int storeId) throws KardexDaoException {
        ...
        try {
            ...
            // construct the SQL statement
            final String SQL = "SELECT * FROM bd_hulkstore.vi_kardex WHERE productId = " + productId + " and storeId = " + storeId;
